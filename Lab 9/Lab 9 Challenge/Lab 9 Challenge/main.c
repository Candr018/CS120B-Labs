...
void set_PWM(double frequency) {
	...
	if (frequency != current_frequency) {
		...
		// prevents OCR0A from underflowing, using pre-scaler 64
...
void PWM_on() {
	...
	// CS01 & CS30: Set a pre-scaler of 64
...
void tick() {
	...
	switch (next_state) //action states
	{
		...
		case measure_1:
		input_frequency = G4;
		button_pressed = button == 0x00 ? 0x00 : 0x01;
		counter++;
		cur_state = 0x01;
		break;
...
int main(void)
{
	...
	DDRB = 0x08; PORTB = 0xF7; // using B3 (0x08) as output to speaker
